...
[Authorize]
public class PartnerPassedChecksModel : PageModel
{
    private readonly IMediator _mediator;

    public PartnerPassedChecksModel(IMediator mediator) => _mediator = mediator;
    ...
    public async Task<IActionResult> OnGetAsync()
    {
        ...
        var pageCountRequest = await _mediator.Send(new GetAllChecksCountByPartnerIdQuery(rdoEnglish_V_Ch.rdoPas_Shode, userFiscalYear, PartnerId, From, To));

        if (pageCountRequest.Succeeded)
        {
            Result.PageInfo = new(CurrentPage, 50, pageCountRequest.Result);
            var result = await _mediator.Send(new GetAllChecksQuery(rdoEnglish_V_Ch.rdoPas_Shode, userFiscalYear, PartnerId, From, To, CurrentPage * 50));
            if (result.Succeeded)
            {
